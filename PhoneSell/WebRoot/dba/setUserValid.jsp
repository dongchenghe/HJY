	...
	<tr>
		...
		<c:forEach items="${users.dataList}" var="dd" varStatus="i" >
			<tr>
				...
				<c:choose>
				<c:when test="${dd.IS_VALID==0}">
					<td><input type="button" value="设为无效" onclick="gaivalid('${dd.USER_ID}','1')">  </td>
				</c:when>
				<c:otherwise>
					<td><input type="button" value="设为有效" onclick="gaivalid('${dd.USER_ID}','0')"></td>
				</c:otherwise>
			</c:choose>
			</tr>
		</c:forEach>
...
function gaivalid(user_id1,value2){
	var user_id = user_id1;
	var value1 = value2;
	layer.confirm('是否确认修改次用户的状态', {
	  btn: ['是','否'] //按钮
	}, function(){
	  xmlHttp.open("POST", "ManagerAjaxUserSet?user_id="+user_id+"&setusername=is_valid&is_valid="+value1, true);
		xmlHttp.setRequestHeader("Context-Type", "application/x-www-form-urlencoded");
		// 服务器返回消息
		xmlHttp.onreadystatechange = function (){
		if(xmlHttp.readyState==4){   
