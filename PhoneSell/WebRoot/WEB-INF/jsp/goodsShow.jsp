			...
			if(xmlHttp.status==200){
				if(xmlHttp.responseText==0){
					...
					if(xmlHttp.responseText==1){
						layer.msg('请登录才能加入购物车哦', {icon: 6});
						window.location.href="ToServlet?jspname=login";
					}else{
        ...
        <div id="mian">
					...
					<font>选择颜色：</font>
					 	<c:forEach items="${colors }" var="co" varStatus="s">
		                    <span name="color1" style=" padding:0px; display:inline-block;height:44px; border:1px solid #DDDDDD; line-height:44px;">
	                			<img style="height:42px; border:1px solid #DDDDDD; line-height:42px;float:left;" src="${goods.imgSrc}/${co.IMG}">
	                			<font>${co.COLOR}</font>
			                </span>
		              	</c:forEach>
				</div>
            ...
            <div id="mystyle" align="center" class="divend">
                ...
                <c:forEach var="i" begin="1" end="6" varStatus="s"> 
                      <img src="${goods.imgSrc}/d${s.index}.png">
                </c:forEach> 
            	</div>
                <div id="di2">
                <c:forEach var="i" begin="1" end="2" varStatus="s"> 
                      <img src="${goods.imgSrc}/g${s.index}.png">
                </c:forEach> 
                </div>
            </div>
            <!--底部-->
